...
(Fig.~\ref{fig:cnn_curves}, Fig.~\ref{fig:cnn_cm}). Validation accuracy
plateaued near 68\% during training, with the best checkpoint saved at epoch
14. The relatively modest result compared to a simpler augmentation regime
reflects the aggressive augmentation pipeline applied during training, which
includes random perspective distortion, random erasing, and color jitter.
These transforms add useful variance for the larger 10-class model but make
convergence harder for the smaller CNN on a clean 5-class dataset. The
confusion matrix shows most errors occurring between visually similar objects
at different scales.
...
reaching 88.8\% test accuracy across 10 classes.
Training from scratch with the CNN achieved 79.7\% on the 5-class dataset,
which is reasonable but reflects the difficulty introduced by the aggressive
augmentation pipeline. The ViT was the slowest to converge and would
