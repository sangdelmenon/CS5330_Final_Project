...
identity rotation and translation. Because the object is assumed to sit on the
optical axis with zero rotation, the back face projects almost concentrically
with the front face at this depth; only a slight perspective scaling
differentiates them. The 12 edges of the box are drawn as colored lines with
each class assigned a unique color, and a semi-transparent label tag showing the
class name and confidence score is placed above the front face using alpha
blending.
...
processing time during the live demo. The reported dataset size of 1419 images
is the file count; 8 web-crawled images failed to decode at DataLoader
iteration time, so the effective test partition used by both the ViT and
MobileNetV2 confusion matrices contains 206 images rather than 214.
...
saved at epoch 14. The gap between validation and test accuracy is primarily
a product of small split sizes. With roughly 120 samples in each held-out
partition, a single random seed can produce a test split that is slightly
easier or harder than the validation split, shifting results by a few points.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{MOBILENET_Loss_Accuracy.png}
...
dramatically to 87.5\% precision and recall under MobileNetV2, compared to
near-zero under the from-scratch ViT, confirming that the pretrained backbone
learned to distinguish eyeglasses despite the noisy web crawl data.
...
the CNN achieved 79.7\% on the 5-class dataset, a reasonable result given the
aggressive augmentation pipeline, though not directly comparable to the 10-class
results from the ViT and MobileNetV2. The ViT reached only 43.2\% on the 10-class dataset when trained
...
parameter count (0.83M for ViT versus 2.19M for the CNN) was not the limiting
